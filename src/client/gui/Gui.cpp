...
void Gui::handleKeyPressed(int key)
{
	...
	{
		// Java semantics: Q drops one of the currently selected hotbar item;
		// Shift+Q drops the whole stack.
		Inventory* inv = minecraft->player->inventory;
		if (Keyboard::isKeyDown(Keyboard::KEY_LSHIFT)) {
			inv->dropSlot(inv->selected, false);
		} else {
			ItemInstance* item = inv->getItem(inv->selected);
			if (item && !item->isNull()) {
				ItemInstance one = *item;
				one.count = 1;
				minecraft->player->drop(new ItemInstance(one), false);
				item->count -= 1;
				if (item->count <= 0) inv->clearSlot(inv->selected);
			}
		}
	}
}
...
void Gui::renderHearts() {
	...
	// HUD layout: hearts directly above the hotbar's left edge, armor row in
	// the same row but anchored to the hotbar's right edge so it sits over
	// the right half of the hotbar (Java-style — hunger would normally be
	// there). The legacy BAR_ON_TOP option keeps the inverted layout
	// (everything anchored to the top of the screen because the hotbar
	// lives there).
	const bool barOnTop = minecraft->options.getBooleanValue(OPTIONS_BAR_ON_TOP);
	const int hotbarLeftX  = screenWidth / 2 - getNumSlots() * 10;
	const int hotbarRightX = screenWidth / 2 + getNumSlots() * 10;
	int xx     = barOnTop ? (hotbarLeftX - 1)   : hotbarLeftX;
	int rowY   = barOnTop ? (screenHeight - 32) : (screenHeight - 22 - 10);

	// Armor row is right-anchored: 10 segments × 8 px = 80 px, ending flush
	// with the right edge of the hotbar.
	const int armorStartX = hotbarRightX - (Player::MAX_HEALTH / 2) * 8;
	...
	for (int i = 0; i < Player::MAX_HEALTH / 2; i++) {
		...
		if (armor > 0) {
			int xo = armorStartX + i * 8;
			if      (ip2 <  armor) blit(xo, rowY, 16 + 2 * 9, 9, 9, 9);
			else if (ip2 == armor) blit(xo, rowY, 16 + 4 * 9, 9, 9, 9);
			else /* ip2 > armor */ blit(xo, rowY, 16 + 0 * 9, 9, 9, 9);
		}
		...
		int xo = xx + i * 8;
		int yo = rowY;
		if (h <= 4) yo += random.nextInt(2) - 1;
		blit(xo, yo, 16 + bg * 9, 0, 9, 9);
		if (blink) {
			if (ip2 < oh)         blit(xo, yo, 16 + 6 * 9, 0, 9, 9);
			else if (ip2 == oh)   blit(xo, yo, 16 + 7 * 9, 0, 9, 9);
		}
		if      (ip2 <  h) blit(xo, yo, 16 + 4 * 9, 0, 9, 9);
		else if (ip2 == h) blit(xo, yo, 16 + 5 * 9, 0, 9, 9);
	}
}
...
void Gui::renderBubbles() {
	if (minecraft->player->isUnderLiquid(Material::water)) {
		...
		// Hearts + armor now share the row directly above the hotbar, so the
		// air-bubble row stacks one row higher (mirrors Java once the
		// hunger bar shows up). Right-anchored so it grows in from the
		// right edge of the hotbar.
		const bool barOnTop = minecraft->options.getBooleanValue(OPTIONS_BAR_ON_TOP);
		const int hotbarRightX = screenWidth / 2 + getNumSlots() * 10;
		int xx = barOnTop ? (screenWidth / 2 - getNumSlots() * 10 - 1)
		                  : (hotbarRightX - 10 * 8);
		int yo = barOnTop ? (screenHeight - 42) : (screenHeight - 22 - 20);
