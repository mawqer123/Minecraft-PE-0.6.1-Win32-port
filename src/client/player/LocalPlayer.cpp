...
#include "../gui/screens/desktop/DesktopChestScreen.h"
#include "../gui/screens/desktop/DesktopFurnaceScreen.h"
	...
	portalCooldown(0)
{
	...
	if (minecraft->options.getStringValue(OPTIONS_USERNAME).size() != 0) {
		...
		// Skin/cape auto-download from Mojang's API disabled — was matching the
		// configured username against real Mojang accounts and applying their
		// skin (anything from a cat to whatever else). Keep the default Steve
		// texture (mob/char.png) for now.
...
void LocalPlayer::tick() {
	...
	// Portal tick detection. When the player is standing inside a portal
	// block, count up; firing point is ~4 seconds (80 ticks). The actual
	// dimension swap is fired AFTER the tick logic finishes so we don't
	// invalidate `level` from underneath ourselves.
	bool triggerPortal = false;
	int  triggerDestDim = 0;
	if (Tile::portalTile && level && minecraft) {
		int px = Mth::floor(x);
		int py = Mth::floor(y);
		int pz = Mth::floor(z);
		int t = level->getTile(px, py, pz);
		if (t == Tile::portalTile->id) {
			portalCooldown++;
			if (portalCooldown >= 80) {
				triggerPortal = true;
				// Return to the dim the level booted with — for Creative that's
				// NORMAL (0), for Survival NORMAL_DAYCYCLE (10). Hardcoding 10
				// here caused leaving the Nether to spawn a *second* overworld
				// instance instead of restoring the stashed original.
				triggerDestDim = (level->dimension && level->dimension->id == Dimension::NETHER)
				    ? level->getHomeDimensionId() : Dimension::NETHER;
				portalCooldown = -100;  // cooldown so we don't re-trigger immediately on the other side
			}
	...
	if (triggerPortal) {
		minecraft->switchDimensionForPlayer(triggerDestDim);
	}
	if(!useItem.isNull()) {
