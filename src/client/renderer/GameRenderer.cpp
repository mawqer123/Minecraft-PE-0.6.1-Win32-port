...
float GameRenderer::getFov(float a, bool applyEffects) {
	...
	float fov = (float)mc->options.getIntValue(OPTIONS_FOV);
	if (fov <= 0) fov = 70;
...
bool GameRenderer::updateFreeformPickDirection(float a, Vec3& outDir) {
	...
	// liquid=true when the held item benefits from picking liquids (currently
	// just an empty bucket — so right-clicking water/lava actually targets it
	// instead of falling through to the block behind). Filled buckets and
	// every other item use the default solid-only pick.
	bool wantLiquid = false;
	if (mc->player && mc->player->inventory) {
		ItemInstance* sel = mc->player->inventory->getSelected();
		if (sel && !sel->isNull() && Item::bucket_empty
			&& sel->id == Item::bucket_empty->id)
		{
			wantLiquid = true;
		}
	}
	const HitResult& hit = mc->hitResult = mc->level->clip(p0, p1, wantLiquid);
