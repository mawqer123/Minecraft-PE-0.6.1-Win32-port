...
void ItemRenderer::renderGuiItem(Font* font, Textures* textures, const ItemInstance* item, float x, float y, float w, float h, bool fancy) {
	...
	if (i < 0) {
		// Flat 2D fallback for tiles that don't have a pre-baked icon in
		// gui_blocks.png (_mapper[id] == -1). The original code path here
		// calls renderGuiItemCorrect which tries a live 3D render with
		// glRotatef/glScalef — that doesn't survive the batched tesselator
		// state used while the inventory is being drawn, so the slot ends
		// up blank. Instead we just blit the tile's top-face texture from
		// terrain.png as a flat icon. It's not isometric like the baked
		// icons, but it shows up and is recognisable.
		if (item->id > 0 && item->id < 256 && Tile::tiles[item->id]) {
			Tile* tile = Tile::tiles[item->id];
			int faceTex = tile->getTexture(1 /* top */, item->getAuxValue());
			if (faceTex < 0) {
				// Some tiles return -1 for top (e.g. cross-shaped). Fall
				// back to the default side texture.
				faceTex = tile->getTexture(2, item->getAuxValue());
				if (faceTex < 0) faceTex = tile->tex;
			}
			textures->loadAndBindTexture("terrain.png");
			float u0 = (float)((faceTex % 16) * 16) / 256.0f;
			float u1 = u0 + 16.0f / 256.0f;
			float v0 = (float)((faceTex / 16) * 16) / 256.0f;
