...
int main(void) {
	...
	// Ask GLFW for OpenGL 1.1. The game uses fixed-function GL plus a few
	// 1.5-era functions (VBOs) that GLAD resolves as extension entry points
	// after context creation, so even a 1.1 base context normally works as
	// long as the driver exports ARB_vertex_buffer_object. This is the
	// minimum that the Microsoft GDI Generic GL driver on old Windows can
	// advertise; without it GLFW outright refuses to make a window on those
	// systems ("driver doesn't seem to support OpenGL"). On Win2K / WinXP
	// VMs ship Mesa's softpipe opengl32.dll next to the exe for full
	// behaviour — the lowered hint just gets you past the GLFW gate.
