    ...
    bool matches(CraftingContainer* craftSlots, int xOffs, int yOffs, bool xFlip) {
        for (int x = 0; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                ...
                // CraftingContainer::getItem never returns NULL — empties come
                // back as a pointer to an isNull() ItemInstance — so treat both
                // forms of "empty" the same way.
                bool itemEmpty = (item == NULL || item->isNull());
                if (itemEmpty && expected.isNull())
                    continue;
                if (itemEmpty ^ expected.isNull())
                    return false;
