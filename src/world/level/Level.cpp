...
Level::~Level() {
	...
	delete dimension;
	// Free any dimensions the player parked via switchDimension. The active
	// _chunkSource/dimension are deleted above; stashed entries cover the
	// inactive dims.
	for (std::map<int, ChunkSource*>::iterator it = _stashedSources.begin(); it != _stashedSources.end(); ++it) {
		delete it->second;
	}
	_stashedSources.clear();
	for (std::map<int, Dimension*>::iterator it = _stashedDimensions.begin(); it != _stashedDimensions.end(); ++it) {
		delete it->second;
	}
	_stashedDimensions.clear();
	delete _pathFinder;
...
void Level::_init(const std::string& levelName, const LevelSettings& settings, int generatorVersion, Dimension* fixedDimension) {
	...
	dimension->init(this);
	// Remember the overworld dim id so the nether<->overworld round-trip
	// targets the same stash key both ways. If we boot directly into the
	// Nether (saved world), the home is the non-Nether default for the game
	// type.
	if (dimension->id == Dimension::NETHER) {
		_homeDimensionId =
			(levelData.getGameType() == GameType::Survival) ? Dimension::NORMAL_DAYCYCLE
			                                                : Dimension::NORMAL;
	} else {
		_homeDimensionId = dimension->id;
	}
	// Tell the storage which dim's chunks file (chunks.dat vs
	// chunks_nether.dat etc.) to read/write before the ChunkCache opens.
	if (levelStorage) levelStorage->setActiveDimensionId(dimension->id);
...
/*public*/
void Level::tick() {
	// The Nether always wants pigmen, regardless of game type. Without this,
	// Creative worlds (where LevelData defaults spawnMobs=false) would have
	// an empty Nether — never seeing the pigman that NetherLevelSource lists.
	bool wantMobSpawn = levelData.getSpawnMobs()
		|| (dimension && dimension->id == Dimension::NETHER);
	if (!isClientSide && wantMobSpawn) {
...
void Level::setInitialSpawn() {
	...
	// the Y discovered during isValidSpawn so we land on real ground.
	int ySpawn = dimension ? dimension->getDefaultSpawnY() : 64;
...
int Level::getSkyDarken(float a) {
	...
	if (br < 0.0f) br = 0.0f;
	// Cap at 1.0 in both fog modes so skyDarken can reach its maximum of 11
	// at midnight. The previous 0.8 cap on the non-Java fog path stopped
	// skyDarken at ~8, which left the effective sky-light level around 7
	// even at the deepest part of the night — too bright for the monster
	// spawn check (`raw <= random[0,8)`) to ever reliably succeed, so mobs
	// almost never spawned at night.
	if (br > 1.0f) br = 1.0f;
	return ((int) (br * 11));
}
...
void Level::switchDimension(int newDimensionId)
{
	int oldDimId = dimension ? dimension->id : -1;
	LOGI("[Level] switchDimension: %d -> %d\n", oldDimId, newDimensionId);

	if (oldDimId == newDimensionId) {
		LOGI("[Level] same dim, nothing to do\n");
		return;
	}

	// Evict non-player entities. They reference chunk objects we're about to
	// park; the level's per-tick loop would dereference dangling chunk pointers
	// through them after the swap. (Persisting entities across the swap is a
	// separate task — for now only block state survives.) Players survive
	// because Minecraft repositions them after the swap.
	{
		EntityList survivors;
		...
		for (size_t i = 0; i < entities.size(); ++i) {
			Entity* e = entities[i];
			if (e) entityIdLookup[e->entityId] = e;
		}
	}

	// Tile entities are owned by chunks; the Level mirror list is rebuilt
	// lazily as chunks tick. Drop our references so nothing left over from
	// the outgoing dim leaks into the new one's tick loop.
	tileEntities.clear();
	pendingTileEntities.clear();

	// Flush the outgoing dim's unsaved chunks to disk now, while
	// levelStorage->activeDimensionId still points at this dim. If we don't,
	// any chunks queued in ExternalFileLevelStorage::unsavedChunkList would
	// be written to the *new* dim's region file on the next tick.
	if (_chunkSource) _chunkSource->saveAll(true);

	// Stash the outgoing dim's chunk source + dimension instance, keyed by
	// id, so a later switch back finds the same chunkMap (with all the
	// player's block edits intact). We never delete them here.
	if (oldDimId >= 0 && _chunkSource && dimension) {
		_stashedSources[oldDimId]    = _chunkSource;
		_stashedDimensions[oldDimId] = dimension;
		LOGI("[Level] stashed dim %d (source=%p dim=%p)\n",
			oldDimId, _chunkSource, dimension);
	} else {
		// Should not happen, but free rather than leak.
		delete _chunkSource;
		delete dimension;
	}
	_chunkSource = NULL;
	...
	// Restore the destination dim if we've been there before, otherwise build
	// it fresh. Restoration brings back every previously-generated chunk
	// (with their block edits) for free, since the chunkMap lives on the
	// stashed ChunkSource.
	std::map<int, Dimension*>::iterator dIt = _stashedDimensions.find(newDimensionId);
	if (dIt != _stashedDimensions.end()) {
		dimension = dIt->second;
		_stashedDimensions.erase(dIt);
		std::map<int, ChunkSource*>::iterator sIt = _stashedSources.find(newDimensionId);
		_chunkSource = sIt->second;
		_stashedSources.erase(sIt);
		// Point storage at this dim's region file for any subsequent save.
		if (levelStorage) levelStorage->setActiveDimensionId(dimension->id);
		LOGI("[Level] restored stashed dim %d (source=%p dim=%p)\n",
			newDimensionId, _chunkSource, dimension);
	} else {
		LOGI("[Level] creating fresh dim %d\n", newDimensionId);
		dimension = Dimension::getNew(newDimensionId);
		if (!dimension) {
			LOGI("[Level] getNew failed, falling back to NORMAL_DAYCYCLE\n");
			dimension = Dimension::getNew(Dimension::NORMAL_DAYCYCLE);
		}
		dimension->init(this);
		// Storage must know the dim id before createChunkSource so that the
		// ChunkCache reads from the right region file (and loads any chunks
		// the player saved here in a previous session).
		if (levelStorage) levelStorage->setActiveDimensionId(dimension->id);
		_chunkSource = createChunkSource();
	}

	// Persist the active dim id to LevelData so the player reopens in the
	// same dim on the next launch. With per-dim region files, each dim's
	// chunks survive disk round-trips independently.
	levelData.setDimension(newDimensionId);
	LOGI("[Level] switchDimension done\n");
}

void Level::saveAllDimensions(bool onlyUnsaved)
{
	int activeDimId = dimension ? dimension->id : 0;

	// Each stashed source has its own ChunkCache holding the chunks for
	// that dim. To route writes to the right per-dim region file, we flip
	// the storage's active id before calling saveAll on the cache, then
	// restore it. The cache iterates its own slot table internally (not
	// via level->getChunk), so it saves the right chunks even though
	// _chunkSource still points at the *active* dim's cache.
	for (std::map<int, ChunkSource*>::iterator it = _stashedSources.begin();
	     it != _stashedSources.end(); ++it)
	{
		if (!it->second) continue;
		if (levelStorage) levelStorage->setActiveDimensionId(it->first);
		it->second->saveAll(onlyUnsaved);
	}

	// Finally, save the active dim with the storage pointed back at it.
	if (levelStorage) levelStorage->setActiveDimensionId(activeDimId);
	if (_chunkSource) _chunkSource->saveAll(onlyUnsaved);
}
