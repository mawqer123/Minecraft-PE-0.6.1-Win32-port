...
public:
	...
	// Swap this Level's active dimension and chunk source in place. All
	// non-player entities and tile entities are evicted (they belong to the
	// old dimension's terrain, which is about to be regenerated). Chunks are
	// destroyed and the new dimension generates fresh terrain on demand.
	// v1 is destructive: state from the old dimension is NOT preserved across
	// the swap; future iterations will save it under DIM-N/ subfolders.
	void switchDimension(int newDimensionId);
protected:
...
protected:
	...
	// In-session stash of chunk sources and dimension instances for dimensions
	// the player has visited but is not currently in. switchDimension parks
	// the outgoing dim here so its chunks (and any tile entities they own,
	// e.g. furnaces, chests) survive the round trip. Disk persistence is not
	// implemented — these maps are wiped when the Level is destroyed.
	std::map<int, ChunkSource*> _stashedSources;
	std::map<int, Dimension*>   _stashedDimensions;
	// The dimension id the level booted with — the "home" dim the player
	// returns to from the Nether. Recorded once at construction; nether
	// portal triggers consult this rather than hardcoding NORMAL_DAYCYCLE,
	// since Creative worlds boot as NORMAL (id 0) and Survival as
	// NORMAL_DAYCYCLE (id 10), and we need the round trip to land on the
	// same id we stashed under.
	int _homeDimensionId;
public:
	int getHomeDimensionId() const { return _homeDimensionId; }
	// Save the active dim's chunks AND every stashed dim's chunks to disk.
	// Each is written to its own per-dim region file (chunks.dat,
	// chunks_nether.dat, ...). Called from Minecraft on game close, save-
	// to-disk events, and similar — replaces the single
	// _chunkSource->saveAll() call which would only persist the active dim.
	void saveAllDimensions(bool onlyUnsaved);
protected:
