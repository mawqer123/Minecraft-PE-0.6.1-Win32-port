...
chunkLoop:
		for(std::map<ChunkPos, bool>::iterator it = chunksToPoll.begin(); it != chunksToPoll.end(); ++it) {
            ...
            // Spawn attempts per polled chunk. Old was 3 outer * 4 inner = 12.
            // Bumped to 5 * 6 = 30 so the spawner has more chances per chunk
            // per tick, especially helpful on the surface where most random
            // Y values land inside solid blocks and get rejected.
            for (int dd = 0; dd < 5; dd++) {
                ...
                for (int ll = 0; ll < 6; ll++) {
                    ...
                    if (isSpawnPositionOk(mobCategory, level, x, y, z)) {
						...
						LOGI("[MobSpawner] spawn type=%d at (%d,%d,%d) dim=%d\n",
							currentMobType.mobClassId, x, y, z,
							level->dimension ? level->dimension->id : -1);
