...
// Per-dim chunks file. The overworld variants (NORMAL id=0, NORMAL_DAYCYCLE
// id=10) keep using the original "chunks.dat" so existing saves remain
// readable. The Nether gets its own file. All other ids would get
// "chunks_dim<id>.dat" — none of those exist yet but we don't want to
// silently lose chunks if a new dim id is added.
RegionFile* ExternalFileLevelStorage::getRegionFile()
{
	std::map<int, RegionFile*>::iterator it = regionFiles.find(activeDimensionId);
	if (it != regionFiles.end() && it->second != NULL) return it->second;

	std::string filename;
	if (activeDimensionId == 0 || activeDimensionId == 10) {
		filename = "chunks.dat";
	} else if (activeDimensionId == 1) {
		filename = "chunks_nether.dat";
	} else {
		char buf[32];
		snprintf(buf, sizeof(buf), "chunks_dim%d.dat", activeDimensionId);
		filename = buf;
	}

	RegionFile* rf = new RegionFile(levelPath, filename);
	if (!rf->open()) {
		LOGI("Failed to open region file %s\n", filename.c_str());
		delete rf;
		regionFiles[activeDimensionId] = NULL;
		return NULL;
	}
	regionFiles[activeDimensionId] = rf;
	return rf;
}
...
void ExternalFileLevelStorage::setActiveDimensionId(int id)
{
	...
	{
		if (it->chunk) save(level, it->chunk);
	}
	unsavedChunkList.clear();
	activeDimensionId = id;
}

// Per-dim entities file. Same naming convention as getRegionFile.
std::string ExternalFileLevelStorage::entitiesFilePathForDim() const
{
	if (activeDimensionId == 0 || activeDimensionId == 10) {
		return levelPath + "/entities.dat";
	}
	if (activeDimensionId == 1) {
		return levelPath + "/entities_nether.dat";
	}
	char buf[64];
	snprintf(buf, sizeof(buf), "/entities_dim%d.dat", activeDimensionId);
	return levelPath + buf;
}
...
void ExternalFileLevelStorage::saveEntities( Level* level, LevelChunk* levelChunk )
{
	...
	FILE* fp = fopen(entitiesFilePathForDim().c_str(), "wb");
	if (fp) {
		int version = 1;
		fwrite("ENT\0", 1, 4, fp);
...
void ExternalFileLevelStorage::loadEntities(Level* level, LevelChunk* chunk) {
	lastSavedEntitiesTick = tickCount;
	FILE* fp = fopen(entitiesFilePathForDim().c_str(), "rb");
	if (fp) {
		char header[5];
				...
				for (int i = 0; i < entityTags->size(); ++i) {
					...
					if (Entity* e = EntityFactory::loadEntity(et, level)) {
						...
						if (typeId == MobTypes::PigZombie) {
							keep = (activeDimensionId == 1); // NETHER only
						} else if (typeId == MobTypes::Chicken
						        || typeId == MobTypes::Cow
						        || typeId == MobTypes::Pig
						        || typeId == MobTypes::Sheep
						        || typeId == MobTypes::Zombie
						        || typeId == MobTypes::Skeleton
						        || typeId == MobTypes::Spider
						        || typeId == MobTypes::Creeper) {
							keep = (activeDimensionId != 1); // not in Nether
						}
						if (keep) {
							level->addEntity(e);
						} else {
