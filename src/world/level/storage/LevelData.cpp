...
void LevelData::setTagData( CompoundTag* tag, CompoundTag* playerTag )
{
	...
	// Nether x/y/z but in the overworld, which usually means falling out
	// of the sky or into terrain.
...
void LevelData::getTagData( const CompoundTag* tag )
{
	...
	// Older saves don't have the Dimension key, so keep the constructor
	// default (NORMAL) if it's missing.
	if (tag->contains("Dimension", Tag::TAG_Int)) {
